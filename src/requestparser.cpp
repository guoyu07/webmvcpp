...
{
    int
    http_request_parser::header_field_cb (http_parser *p, const char *buf, size_t len)
    {
    ...
    int
    http_request_parser::header_value_cb (http_parser *p, const char *buf, size_t len)
    {
        http_request_parser *_this = static_cast<http_request_parser *>(p->data);
    ...
    int
    http_request_parser::request_url_cb (http_parser *p, const char *buf, size_t len)
    {
        http_request_parser *_this = static_cast<http_request_parser *>(p->data);
        ...
        if (urlParamsPos!=std::string::npos)
        {
            ...
            std::vector<std::string> paramsList = split_string(request.queryString, '&');
    ...
    /*
    int
    HttpRequestParser::response_status_cb (http_parser *p, const char *buf, size_t len)
    {
        return 0;
    }
    */
    ...
    int
    http_request_parser::body_cb (http_parser *p, const char *buf, size_t len)
    {
        http_request_parser *_this = static_cast<http_request_parser *>(p->data);
        append_bytes(_this->request.content, (const unsigned char *)buf, len);

        return 0;
    }

    int
    http_request_parser::message_begin_cb (http_parser *p)
    {
        (void)p;
    ...
    int
    http_request_parser::headers_complete_cb (http_parser *p)
    {
        http_request_parser *_this = static_cast<http_request_parser *>(p->data);
        ...
        if (realmIt != request.header.end())
        {
            ...
            if (authParamSepPos != std::string::npos)
            {
                ...
                if (auth1param=="basic")
                {
                    ...
                    std::string encodedRealm = base64_decode(auth2param);
...
*/
        ...
        if (hostIt != request.header.end()) {
            request.host = split_string(hostIt->second, ':')[0];
        }
        ...
        const std::vector<std::string> splittedGetParams = split_string(request.queryString, '&');
    ...
    int
    http_request_parser::message_complete_cb (http_parser *p)
    {
        http_request_parser *_this = static_cast<http_request_parser *>(p->data);
        ...
        if (request.contentType.find("application/x-www-form-urlencoded")!=std::string::npos)
        {
            ...
            const std::vector<std::string> splittedPostParams = split_string(paramsStr, '&');
    ...
    http_request_parser::http_request_parser(http_request & httpReq):
	webmvcobject(),
    request(httpReq),
    ...
    int http_request_parser::accept_data(const unsigned char * buffer, unsigned int length)
    {
